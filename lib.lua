...
local library = {
	...
	colors = {
		main = Color3.fromRGB(0, 255, 140),
		background = Color3.fromRGB(18, 24, 22),
		outerBorder = Color3.fromRGB(8, 14, 12),
		innerBorder = Color3.fromRGB(50, 130, 90),
		topGradient = Color3.fromRGB(22, 32, 28),
		bottomGradient = Color3.fromRGB(16, 24, 21),
		sectionBackground = Color3.fromRGB(20, 30, 26),
		section = Color3.fromRGB(210, 210, 210),
		otherElementText = Color3.fromRGB(180, 180, 180),
		elementText = Color3.fromRGB(200, 200, 200),
		elementBorder = Color3.fromRGB(10, 18, 15),
		selectedOption = Color3.fromRGB(40, 95, 70),
		unselectedOption = Color3.fromRGB(20, 35, 28),
		hoveredOptionTop = Color3.fromRGB(45, 110, 80),
		unhoveredOptionTop = Color3.fromRGB(35, 80, 60),
		hoveredOptionBottom = Color3.fromRGB(35, 90, 65),
		unhoveredOptionBottom = Color3.fromRGB(25, 65, 45),
				...
				if getsynasset then
					...
					elseif (#image > 14) and (string.sub(image, 1, 14) == "synasseturl://") then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Axios Lib")
							if fold then
							else
								makefolder("./Axios Lib")
							end
							fold = isfolder("./Axios Lib/Themes")
							if fold then
							else
								makefolder("./Axios Lib/Themes")
							end
							fold = isfolder("./Axios Lib/Themes/SynapseAssetsCache")
							if fold then
							else
								makefolder("./Axios Lib Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Axios Lib/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Axios Lib/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Axios Lib/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "Axios Lib" then
		library.WorkspaceName = convertfilename(windowName, "Axios Lib")
	end
	local axiosLibrary = Instance_new("ScreenGui")
	library.MainScreenGui, MainScreenGui = axiosLibrary, axiosLibrary
	...
	axiosLibrary.Name = "     "
	axiosLibrary.Parent = library.gui_parent
	...
	function windowFunctions:CreateTab(options, ...)
		options = (options and (type(options) == "string") and resolvevararg("Tab", options, ...)) or options or {
			Name = "Axios Style: Elite Lego Hax"
		}
		...
		function tabFunctions:CreateSection(options, ...)
				...
				local function AddOptions(optionsTable, filter)
					...
					for _, v in next, optionsTable do
						if not filter or tostring(v):lower():find(dropdownSelection.Text:lower(), 1, not options.RegEx) then
							...
							library.signals[1 + #library.signals] = optionButton[(multiselect and "MouseButton1Click") or "Activated"]:Connect(function()
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Axios Lib") then
						makefolder("./Axios Lib")
					end
					local common_string = "./Axios Lib/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Axios Lib/" .. tostring(custom_workspace or library.WorkspaceName)
						end
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			Name = "Image Asset ID",
			Placeholder = "rbxassetid://73509622124396",
			Flag = "__Designer.Background.ImageAssetID",
			Value = "rbxassetid://73509622124396",
